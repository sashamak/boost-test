...
# Start from requesting a command-line argument.
# Request single argument with the configuration file url.
...
def fetch_config(url):
    ...
    config["input_file_data"] = requests.get(urljoin(url,
                                             config["input_file_name"])).json()
...
def process(data, output_file_name):
    ...
    with open(output_file_name, "w", newline="") as file:
        ...
        for id in boxes:
            values = np.array([(coords[2]-coords[0])*(coords[3]-coords[1])
                              for coords in boxes[id]])
            writer.writerow([id,
                             np.average(values),
                             np.min(values),
                             np.max(values)])
